...
void UART_Init()
{
  ...
  pUartHandleArray[UART_TENSION] = &huart6;

  //gUarts[UART_ESP12];
  //gUarts[UART_TEMP_HUM];
  //gUarts[UART_DUST];

  //gUarts[UART_TENSION].useReceiveTimeout = 1;
  //gUarts[UART_TENSION].isReceived = 0;
  //gUarts[UART_TENSION].lastReceiveTime = 0;
  //gUarts[UART_TENSION].receiveTimout = UART_RECEIVE_TIMEOUT;
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	else if(USART3 == huart->Instance)  pUartQ = &gUarts[UART_TENSION];
...
void UART_RX_DefaultProc(void)
{
	...
	if (0 == CQ_IsFull(pRxQ)) {
		...
		for(uint32_t j=0; j<_receiveCount; j++) {
			...
			if (UART_LF == _ch) {
				//CDC_Transmit_FS(pUartQ->buffer, pUartQ->bufferIndex);
				ParsingTempHum(pUartQ->buffer, pUartQ->bufferIndex);
				pUartQ->bufferIndex = 0;
			}
		}
	}	
	...
	if (0 == CQ_IsFull(pRxQ)) {
        ...
        for(uint32_t j=0; j<_receiveCount; j++) {
			...
			if (UART_LF == _ch) {
				//CDC_Transmit_FS(pUartQ->buffer, pUartQ->bufferIndex);
				ParsingTension(pUartQ->buffer, pUartQ->bufferIndex);
				pUartQ->bufferIndex = 0;
			}
		}
	}
	...
	if (0 == CQ_IsFull(pRxQ)) {
        ...
        for(uint32_t j=0; j<_receiveCount; j++) {
			...
			if (UART_LF == _ch) {
				//CDC_Transmit_FS(pUartQ->buffer, pUartQ->bufferIndex);
				ParsingUV(pUartQ->buffer, pUartQ->bufferIndex);
				pUartQ->bufferIndex = 0;
			}
		}
	}
	...
	if (0 == CQ_IsFull(pRxQ)) {
		...
		for(uint32_t j=0; j<_receiveCount; j++) {
			...
			if (MAX_DUST_DATA_COUNT == pUartQ->bufferIndex) {
				//CDC_Transmit_FS(pUartQ->buffer, pUartQ->bufferIndex);
				ParsingDust(pUartQ->buffer, pUartQ->bufferIndex);
				pUartQ->bufferIndex = 0;
			}
		}
	}
